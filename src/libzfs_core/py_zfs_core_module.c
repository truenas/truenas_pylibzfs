...
"local_replicate(*, source, dest, fromsnap=None, send_flags=0, props=None, force=False, raw=False) -> None\n"
"---------------------------------------------------------------------------------------------------------\n\n"
"Replicate a snapshot to another location in the same kernel by streaming\n"
"a send stream into a receive in a single call.  An internal pipe carries\n"
"the data between a background send and the calling-thread receive.  The\n"
"GIL is released for the duration of both ioctls so other Python threads\n"
"can run.\n\n"
"Parameters\n"
"----------\n"
"source: str, required\n"
...
"------\n"
"ValueError:\n"
"    source or dest was omitted, or send_flags contained RAW or\n"
"    SAVED.\n"
"TypeError:\n"
"    props is not a dictionary.\n"
"OSError:\n"
"    The internal pipe could not be created or the background send\n"
"    thread could not be started.\n"
"ZFSCoreException:\n"
"    lzc_send() or lzc_receive() returned a non-zero errno.  When\n"
"    both sides fail, the receive error is reported because it is\n"
"    almost always the underlying cause (a failed receive closes the\n"
"    pipe and causes a downstream EPIPE on the send side).\n"
"RuntimeError:\n"
"    Failed to log history.\n\n"
"Notes\n"
"-----\n"
"This call is not interruptible from Python while a transfer is in\n"
"progress.  The underlying lzc_send and lzc_receive ioctls block in\n"
"the kernel; SIGINT (Ctrl-C) cannot raise KeyboardInterrupt during\n"
"the call.  Where in the kernel a thread is waiting determines what\n"
"the caller actually observes:\n"
"  - At a signal-aware wait point (cv_wait_sig / cv_wait_io_sig in\n"
"    the send or receive code path), the ioctl returns EINTR and is\n"
"    surfaced as ZFSCoreException with errno == EINTR.\n"
"  - At an uninterruptible wait, the signal is held pending until\n"
"    the ioctl returns naturally; KeyboardInterrupt is then raised\n"
"    on the next bytecode in the calling Python frame.\n"
"To cancel a long-running replication that is not responding to\n"
"signals, send SIGKILL to the process.\n"
);
