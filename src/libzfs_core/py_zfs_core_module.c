...
"                force=False, raw=False, resumable=False,\n"
"                resume_token=None, progress_callback=None,\n"
...
"    agree.\n"
"resumable: bool, optional, default=False\n"
"    If True, the receive side is run in resumable mode: a partial\n"
"    transfer leaves a RECEIVE_RESUME_TOKEN property on the\n"
"    destination so a subsequent call with resume_token can pick up\n"
"    where this one stopped.  Without this flag, an interrupted\n"
"    receive cleans up after itself and no token is produced.\n"
"resume_token: str | None, optional, default=None\n"
"    If set, resume a previously interrupted send instead of\n"
"    starting fresh.  Read the token off the partial destination\n"
"    via ZFSProperty.RECEIVE_RESUME_TOKEN.  source / fromsnap /\n"
"    send_flags must match the snapshot encoded in the token; the\n"
"    kernel rejects mismatches.  When resuming, no progress total\n"
"    is computed; the callback's `total` argument stays at 0.\n"
"    Resume tokens only address a single (dataset, snapshot) pair,\n"
"    which is why this option is only on lzc.local_replicate;\n"
"    ZFSDataset.local_replicate is recursive and incompatible with\n"
"    resume.\n"
