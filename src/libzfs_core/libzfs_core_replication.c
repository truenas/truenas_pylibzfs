...
py_lzc_local_replicate(PyObject *self, PyObject *args_unused, PyObject *kwargs)
{
	 ...
	 * the read end, which causes the sender to fail with EPIPE/EINTR;
	 * that downstream send error is uninteresting and would mask the
	 * real cause.  Only surface the send error when the receive
	 * succeeded.
	 */
	if (recv_err) {
		snprintf(ctx_msg, sizeof(ctx_msg),
