...
| `py_zfs_pool_create.c` | Pool creation vdev-spec builder, the topology checks shared by `create_pool` and `add_vdevs` (structural rules always, policy rules unless `force=True`, both runnable without side effects via `dry_run=True`) and the `zpool_create` / `zpool_add` / `zpool_import_props` wrappers |
