...
typedef struct {
	...
	const char *key;  // crypto key if keylocation is prompt
	Py_ssize_t key_len;,
...
static
boolean_t validate_keylocation(PyObject *py_keyloc, zfs_crpyto_change_info_t *info)
{
	const char *key_location_uri;
			     ...
			     zfs_crypto_change_info_t *info);
{
	...
	if ((py_keyformat == NULL) || (py_keyformat == Py_NONE)) {
		PyErr_SetString(PyExc_ValueError,
...
boolean_t validate_key_material(PyObject *py_key, zfs_crypto_change_info_t *info)
{
	...
	PYZFS_ASSERT(info->format < ZFS_KEYFORMAT_FORMATS);
	...
	case ZFS_KEYFORMAT_RAW:
		...
		if (PyBytes_AsStringAndSize(py_key, &key, &keylen))
			return B_FALSE;
	...
	case ZFS_KEYFORMAT_HEX:
		...
		Py_DECREF(tmp);
		key = PyUnicode_AsUTF8AndSize(py_key, &keylen);
		if (key == NULL)
			return B_FALSE;
	...
	case ZFS_KEYFORMAT_PASSPHRASE:
		...
		if (keylen < MIN_PASSPHRASE_LEN) {
			...
			return B_FALSE;
		} else if (keylen > MAX_PASSPHRASE_LEN) {
			PyErr_Format(PyExc_ValueError,
				    ...
				    zfs_crypto_change_info_t *info)
{
	...
	if (!validate_keyformat(py_keyformat))
		return B_FALSE;
				...
				PyObject *kwargs)
{
	...
	if (!is_encroot && (info.key_format == ZFS_KEYFORMAT_NONE)) {
		PyErr_SetString(PyExc_ValueError,
						 ...
						 nvlist_t *props)
{
	...
	dump = py_dump_nvlist(props, B_TRUE);
	if (dump != NULL) {
...
static boolean_t pyzfs_create_crypto_key(py_zfs_t *pyzfs,
					 const char *name,
					 zfs_type_t ztype,
					 nvlist_t *props,
					 zfs_crypto_change_info_t *info)
{
	...
	if (success) {
		...
		info->key_location_uri = pbuf;
		crytpo_props = get_change_key_params(info);
		info->key_location_uri = NULL;
		...
		if (err) {
			...
			// to "prompt"
			tmp_hdl = zfs_open(name, ztype);
			if (tmp_hdl) {
				zfs_prop_set(obj->zhp, keyprop, "prompt");
			}
		}
	...
	success = pyzfs_zfs_create_crypto_history(self, name, props);
