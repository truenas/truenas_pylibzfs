...
/* py_zfs_local_replicate.c */
PyDoc_STRVAR(py_zfs_local_replicate__doc__,
"local_replicate(*, tosnap, dest, fromsnap=None, send_flags=0, props=None,\n"
"                exclude_props=None, force=False, raw=False, nomount=False,\n"
"                include_intermediates=False, progress_callback=None,\n"
"                progress_state=None, progress_interval_seconds=1) -> None\n"
"------------------------------------------------------------------\n\n"
"Replicate a snapshot of this resource to another location on the\n"
"same system in a single call.\n\n"
"For a filesystem this mirrors\n"
"`zfs send -Rp [-w] <self>@<tosnap> | zfs receive [-F] <dest>`:\n"
"the send is recursive, so descendant filesystems / volumes / clones\n"
"and their snapshots are included in the stream.\n\n"
"For a volume the send is non-recursive (volumes have no non-snapshot\n"
"descendants) and mirrors\n"
"`zfs send -p [-w] <self>@<tosnap> | zfs receive [-F] <dest>`: just\n"
"the named snapshot is transported.  Use\n"
"`truenas_pylibzfs.lzc.local_replicate` if you want a single-snapshot\n"
"send without `-p`.\n\n"
"Source dataset properties (locally-set and received) are always\n"
"embedded in the stream and applied to the destination.  Note that\n"
"`mountpoint` is among the embedded properties, so replicating to\n"
"the same host typically needs `props={\"mountpoint\": \"...\"}` or\n"
"`props={\"canmount\": \"noauto\"}` to avoid mount collisions on the\n"
"destination.\n\n"
"Parameters\n"
"----------\n"
"tosnap: str, required\n"
"    Name (suffix only) of the snapshot of this dataset to send.\n"
"    The full source is f\"{self.name}@{tosnap}\".  This snapshot\n"
"    must already exist on this dataset; it is validated up front.\n"
"dest: str, required\n"
"    Destination dataset name (e.g. \"backup/foo\").  The receive\n"
"    creates dest@tosnap; for a filesystem source, descendants\n"
"    present on the source are created on the destination too.\n"
"fromsnap: str | None, optional, default=None\n"
"    Suffix-only name of the incremental base snapshot.  When\n"
"    provided, only the delta from fromsnap to tosnap is sent\n"
"    (`zfs send -i`).  Pair with include_intermediates=True to\n"
"    request `zfs send -I` semantics instead.\n"
"send_flags: int, optional, default=0\n"
"    Bitwise combination of SendFlags values that control the wire\n"
"    format of the stream: EMBED_DATA, LARGE_BLOCK, COMPRESS.\n"
"    SendFlags.RAW must be set via the raw=True keyword and is\n"
"    rejected here; SendFlags.SAVED is not supported.\n"
"props: dict | None, optional, default=None\n"
"    Property overrides applied to the destination, equivalent to\n"
"    `zfs receive -o key=value`.  Wins over the source's embedded\n"
"    properties via the LOCAL-beats-RECEIVED layering the kernel\n"
"    applies.\n"
"exclude_props: Iterable[str] | None, optional, default=None\n"
"    Property names whose embedded source values should NOT be\n"
"    applied to the destination (equivalent to `zfs receive -x prop`).\n"
"    The destination inherits the property from its parent rather\n"
"    than receiving the source's value.  Overlapping with `props`\n"
"    on the same name is rejected with ValueError.\n"
"force: bool, optional, default=False\n"
"    If True, roll back the destination to its most recent snapshot\n"
"    before receiving (equivalent to `zfs receive -F`).\n"
"raw: bool, optional, default=False\n"
"    If True, send and receive raw on-disk records (equivalent to\n"
"    `zfs send -w`).  Required for replicating an encrypted dataset\n"
"    without loading its wrapping key.\n"
"nomount: bool, optional, default=False\n"
"    If True, do not mount the destination after receive (equivalent\n"
"    to `zfs receive -u`).  By default zfs_receive auto-mounts new\n"
"    filesystems; set this when replicating into a path that you\n"
"    intend to manage mounts for separately, or when the destination\n"
"    inherits a `mountpoint` that would collide with an existing\n"
"    mount on the same host.\n"
"include_intermediates: bool, optional, default=False\n"
"    If True, the stream includes every snapshot between fromsnap and\n"
"    tosnap rather than a single delta (equivalent to `zfs send -I`).\n"
"    Requires fromsnap; ValueError otherwise.  On a filesystem this\n"
"    composes with the implicit -R to give `zfs send -RI` semantics,\n"
"    so descendants get their intermediate snapshots replicated too.\n"
"    Useful for catching a destination up through every intermediate\n"
"    snapshot rather than collapsing them into one diff.\n"
"progress_callback: Callable[[int, int, Any], None] | None, optional\n"
"    If provided, called periodically while the transfer is in\n"
"    flight with (bytes_sent, total, progress_state).  For this\n"
"    method total is always 0 - the resource path does not\n"
"    estimate the total size up front; render accordingly.\n"
"progress_state: Any, optional, default=None\n"
"    Opaque object passed through verbatim to progress_callback.\n"
"progress_interval_seconds: int, optional, default=1\n"
"    Minimum seconds between progress_callback invocations.\n\n"
"Returns\n"
"-------\n"
"None\n\n"
"Raises\n"
"------\n"
"ValueError:\n"
"    tosnap or dest was omitted, send_flags contained RAW or SAVED,\n"
"    progress_interval_seconds was not a positive integer, a property\n"
"    name appears in both props and exclude_props, or\n"
"    include_intermediates was set without fromsnap.\n"
"TypeError:\n"
"    props is not a dictionary, exclude_props is not iterable of\n"
"    strings, or progress_callback is not callable.\n"
"OSError:\n"
"    Failed to set up the internal transport between send and receive.\n"
"ZFSException:\n"
"    The pre-flight snapshot lookup failed, the send or receive\n"
"    failed, or history logging failed.\n"
);
...
/* provided by py_zfs_pool_expand.c */
extern void init_py_zpool_expand_state(pylibzfs_state_t *state);
extern PyObject *py_get_pool_expand_info(py_zfs_pool_t *p);
